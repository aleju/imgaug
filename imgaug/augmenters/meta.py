...
class Augmenter(object): # pylint: disable=locally-disabled, unused-variable, line-too-long
    ...
    def augment_heatmaps(self, heatmaps, parents=None, hooks=None):
        ...
        Returns
        -------
        heatmap_result : list of ia.HeatmapsOnImage
    ...
    def _augment_heatmaps(self, heatmaps, random_state, parents, hooks):
        ...
        Returns
        ----------
        images : list of ia.HeatmapsOnImage
    ...
    @staticmethod
    def wrap_bounding_boxes_on_image(bounding_box_matrix, image_shape):
        """
        Makes the Bounding Box construction a one-liner.
        It wraps the bounding box numpy array with ia.BoundingBoxes and ia.BoundingBoxesOnImage objects
        ...
        image_shape: Shape of the image: (Height, Width, Channels) OR (Height, Width)

        Returns
        -------
        result: ia.BoundingBoxesOnImage
        """
        ...
        for box in bounding_box_matrix:
            x1, y1, x2, y2 = box
            tmp_box = ia.BoundingBox(x1, y1, x2, y2)
            boxes.append(tmp_box)

        return ia.BoundingBoxesOnImage(boxes, image_shape)

    @staticmethod
    def unwrap_bounding_boxes_on_image(bounding_boxes_on_image, bounding_box_dtype=np.int16):
        """
        Unwraps ia.BoundingBoxes inside a ia.BoundingBoxesOnImage object to a simple Numpy array

        Parameters
        ----------
        bounding_boxes_on_image: ia.BoundingBoxesOnImage object
        bounding_box_dtype: Numpy data type of the returned array
        ...
        boxes = bounding_boxes_on_image.bounding_boxes

        bounding_box_matrix = np.zeros((len(boxes), 4), dtype=bounding_box_dtype)
...
class Lambda(Augmenter):
    ...
    func_heatmaps : callable,
        ...
        This is essentially the interface of `Augmenter._augment_heatmaps()`.
...
def AssertShape(shape, check_images=True, check_heatmaps=True, check_keypoints=True, name=None, deterministic=False, random_state=None):
    ...
    check_heatmaps : bool, optional(default=True)
